...
export function GameCard3D({
  ...
  return (
    <div
      ...
      className={`group relative p-3 rounded-3xl bg-surface/90 border-2 transition-all duration-300 transform hover:-translate-y-2.5 active:scale-95 cursor-pointer flex flex-col justify-between overflow-hidden shadow-2xl ${styles.border} ${styles.glow}`}
    >
      ...
      <div className="flex flex-col gap-2.5 px-1 pb-1">
        <div className="flex items-center justify-between gap-2 text-xs font-extrabold text-foreground/80 bg-white/5 border border-white/10 px-3 py-1.5 rounded-xl">
          <span className="flex items-center gap-1.5">
            <Users size={14} className="text-primary" /> {playersMin}-{playersMax} joueurs
          </span>
          <span className="text-white/20">•</span>
          <span className="flex items-center gap-1.5">
            <Clock size={14} className="text-primary" /> {durationMins} min
          </span>
        </div>

        {/* Bouton 3D Tactile Jouer */}
